...
void setup()   { 
  ...
  // Set them to 0 V
//  Timer1.pwm(PS1_V_OUT, 0);
//  Timer1.pwm(PS1_I_OUT, 0);
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (keyword == 'c') {
        ...
        for (int channelIndex = 0; channelIndex < numberOfChannels; channelIndex++) { 
          ...
          if (channelIdentifiers[channelIndex] == 'v') {
            ...
            if (channelNumbers[channelIndex] == 0){
              valueToOutput = analogRead(PS1_V_IN) * 5.0 / 1023;
              Serial.println("Reading PS1 V: "+(String)valueToOutput + " V)");
            }
            else if (channelNumbers[channelIndex] == 1){
              valueToOutput = analogRead(PS2_V_IN) * 5.0 / 1023;
              Serial.println("Reading PS1 V: "+(String)valueToOutput + " V)");
            }
          }
          else if (channelIdentifiers[channelIndex] == 'i') {
            // read 'a' for analog in 
            if (channelNumbers[channelIndex] == 0){
              valueToOutput = analogRead(PS1_I_IN) * 5.0 / 1023;
              Serial.println("Reading PS1 V: "+(String)valueToOutput + " V)");
            }
            else if (channelNumbers[channelIndex] == 1){
              valueToOutput = analogRead(PS2_I_IN) * 5.0 / 1023;
              Serial.println("Reading PS1 V: "+(String)valueToOutput + " V)");
            }
          }
      ...
      if (channelIdentifier == 'v') {
        
        if (channelNumber == '1'){

          Serial.println("Setting PS1 V: "+(String)valueToSetNumerical + " ("+(String)(valueToSetNumerical*5./255.)+"V)");
          analogWrite(PS1_V_OUT, valueToSetNumerical);
          analogRead(PS1_V_IN);
          Serial.println(analogRead(PS1_V_IN) * 5.28 / 1023.0);
          // Timer1.pwm(PS1_V_OUT, valueToSetNumerical);

        }  
           
        else if (channelNumber == '2'){

          Serial.println("Setting PS2 V: "+(String)valueToSetNumerical + " ("+(String)(valueToSetNumerical*5./255.)+"V)");
          analogWrite(PS2_V_OUT, valueToSetNumerical);
          analogRead(PS2_V_IN);
          Serial.println(analogRead(PS2_V_IN) * 5.28 / 1023.0);
        }
      
      }
      else if (channelIdentifier == 'i') {
        
        if (channelNumber == '1'){

          Serial.println("Setting PS1 I: "+(String)valueToSetNumerical + " ("+(String)(valueToSetNumerical*5./255.)+"V)");
          analogWrite(PS1_I_OUT, valueToSetNumerical);
          analogRead(PS1_I_IN);
          Serial.println(analogRead(PS1_I_IN) * 5.28 / 1023.0);
          // Timer1.pwm(PS1_I_OUT, valueToSetNumerical);

        }  
           
        else if (channelNumber == '2'){

          Serial.println("Setting PS2 I: "+(String)valueToSetNumerical + " ("+(String)(valueToSetNumerical*5./255.)+"V)");
          analogWrite(PS2_I_OUT, valueToSetNumerical);
          analogRead(PS2_I_IN);
          Serial.println(analogRead(PS2_I_IN) * 5.28 / 1023.0);
        }
      }
    }
  } 
